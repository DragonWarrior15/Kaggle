...
merged_dataset_train = pd.merge(activity_dataset_2, people_dataset_2, on = 'people_id', how = 'inner')
merged_dataset_train.to_csv('merged_dataset_test.csv', index = False)
'''

'''
print('getting no of rows')
f = open('merged_dataset_train.csv', 'r')
nrows = -1
for line in f:
	nrows = nrows + 1
f.close()
...
'''
f = open('merged_dataset_train_random.csv', 'w')
f.write('merged_dataset_train_random\n')
...
f_w = open('merged_dataset_train_0.1.csv', 'w')
f_r_1 = open('merged_dataset_train.csv', 'r')
f_r_2 = open('merged_dataset_train_random.csv', 'r')
...
input_file = 'merged_dataset_train_not_type1.csv'
type1 = False

merged_dataset_train = pd.read_csv(input_file)
#get important info from months
print("Processing dates")
for col in ["pattr_date", "aattr_date"]:
	merged_dataset_train[col + "_month"] = merged_dataset_train.apply(lambda row : datetime.datetime.strptime(str(row[col]), "%Y-%m-%d").date().month, axis = 1)
	merged_dataset_train[col + "_day"] = merged_dataset_train.apply(lambda row : datetime.datetime.strptime(str(row[col]), "%Y-%m-%d").date().day, axis = 1)
	merged_dataset_train[col + "_weekday"] = merged_dataset_train.apply(lambda row : datetime.datetime.strptime(str(row[col]), "%Y-%m-%d").date().weekday(), axis = 1)
print("Dates processed")
#merged_dataset_test = pd.read_csv('merged_dataset_test.csv')

#print(merged_dataset_train.dtypes)
...
if type1:
	merged_dataset_train = get_woe(cat_var_list_type1, 'outcome', merged_dataset_train, 'woe_type1.csv')
else:
	merged_dataset_train = get_woe(cat_var_list_not_type1, 'outcome', merged_dataset_train, 'woe_not_type1.csv')
...
for col in bool_var_list:
	merged_dataset_train['ind_' + col] = merged_dataset_train.apply(lambda row : 1 if row[col] else 0, axis = 1)
...
#merged_dataset_train['ind_aattr_char_6_type1'] = merged_dataset_train.apply(lambda row : 1 if row['aattr_char_6'] == 'type 1' else 0, axis = 1)
all_col_list = merged_dataset_train.columns.values.tolist()
...
merged_dataset_train[keep_col_list].to_csv(path_or_buf = input_file[:-4] + '_mod.csv', index = False)
#merged_dataset_test[keep_col_list].to_csv(path_or_buf = 'merged_dataset_test_type1_mod.csv', index = False)



#get the correlation matrix
#pd.read_csv('merged_dataset_train_0.1_not_type1_mod.csv').corr().to_csv('corr_train_not_type1.csv')

'''
merged_dataset_train = pd.read_csv('merged_dataset_train_0.1.csv')
merged_dataset_train = merged_dataset_train[merged_dataset_train['aattr_activity_category'] != 'type 1']
merged_dataset_train.to_csv('merged_dataset_train_0.1_not_type1.csv', index = False)

merged_dataset_train = pd.read_csv('merged_dataset_train_0.1.csv')
merged_dataset_train = merged_dataset_train[merged_dataset_train['aattr_activity_category'] == 'type 1']
merged_dataset_train.to_csv('merged_dataset_train_0.1_type1.csv', index = False)
'''