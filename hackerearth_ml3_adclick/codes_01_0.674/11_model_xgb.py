...
for param_list in param_space:
    ...
    clf = XGBClassifier(max_depth     = param_list[param_to_int_dict['max_depth']], 
                        ...
                        gamma         = param_list[param_to_int_dict['gamma']]
                        )


    clf.fit(X[:,c_vars.col_index_training[:i]], y)
    print (str(datetime.now()) + ' Model Training Complete')


    print (str(datetime.now()) + ' Making Predictions on train set')
    # predict on the test set
    y_pred = clf.predict(X[:,c_vars.col_index_training[:i]])
    y_pred_proba = clf.predict_proba(X[:,c_vars.col_index_training[:i]])
    ...
    # predict on the test set
    y_test_pred = clf.predict(X_test[:,c_vars.col_index_training[:i]])
    y_test_pred_proba = clf.predict_proba(X_test[:,c_vars.col_index_training[:i]])
...
# y_submit_pred_proba = clf.predict_proba(X_submit[:,c_vars.col_index_training[:i]])
