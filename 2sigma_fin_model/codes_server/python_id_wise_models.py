...
id_test_list = np.unique(X_test['id'])

for id_key in id_train_list:
  id_mean_dict[id_key] = X_train[X_train['id'] == id_key].mean(axis = 0)
  X_train[X_train['id'] == id_key].fillna(id_mean_dict[id_key], inplace = True)
  if id_key in id_test_list:
    X_test[X_test['id'] == id_key].fillna(id_mean_dict[id_key], inplace = True)
...
for id_key in id_train_list:
  # model = Ridge(alpha = 0.07) #-0.0002
  # model = LinearRegression() # 0.0019
  model = ExtraTreesRegressor(n_estimators = 100, max_depth = 4, n_jobs = -1, random_state = 100000000)
